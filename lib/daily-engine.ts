...
const PROFILE = {
  ...
  "entityLabel": "State",
  "detailSegment": "state",
  // 2026-07-26 (2차) — 도시 축 판정이 났다: cities 194행은 미출처 편집 추정치라 전량 410,
  // lib/db.ts 도 삭제. 그래서 dataModule 을 states-data 로 옮긴다 — 여기 51행은 Zillow ZHVI
  // Apr 2025 · FHFA HPI · Census ACS 2023 · Tax Foundation 2023 · NAIC 2023 · FRED
  // MORTGAGE30US 에 필드별로 앵커돼 있다. metricFields 를 명시하는 이유: 제네릭
  // "첫 숫자 컬럼" 폴백이 StateData 의 앞쪽 필드를 집어 순위 의미가 흐려진다.
  "source": "Zillow ZHVI (Apr 2025) · FHFA HPI · Census ACS 2023",
  // 모든 item 이 medianHomePrice 를 보여주므로 1차 앵커의 빈티지를 쓴다(ZHVI 2025-04).
  "dataDate": "2025-04",
  "dataModule": "states-data",
  "dataFn": "getStatesSortedByPrice",
  "metricFields": ["medianHomePrice", "yoyChange", "affordabilityIndex"]
} as {
  id: string; name: string; domain: string; entityLabel: string;
  detailSegment?: string; metricFields?: string[]; dataModule?: string; dataFn?: string;
...
export async function generateDailyEntry(date = todayKst()): Promise<DailyEntry> {
  ...
  return writeFingerprint({
    ...
    // 2026-07-26 — `dataDate: date` 였다. 페이지는 이걸 "Data date: X" 로 렌더하는데,
    // 값은 발행일이라 Apr 2025 ZHVI 를 오늘 자 데이터라고 공지하고 있었다. 사이트가
    // 실제 빈티지를 아는 경우 그걸 쓴다. (같은 엔진 사본이 함대 61곳에 더 있다 — 그쪽은
    // PROFILE.dataDate 가 없어 발행일 폴백 그대로다.)
    dataDate: PROFILE.dataDate ?? date,
  });
}
